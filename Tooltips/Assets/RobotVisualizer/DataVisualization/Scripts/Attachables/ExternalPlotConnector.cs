...
namespace RobotVisualizer
{
    ...
    /// </summary>
    [ExecuteAlways]
    [AddComponentMenu("Scripts/MRTK/SDK/ExternalPlotConnector")]
    public class ExternalPlotConnector: MonoBehaviour
    {
        [SerializeField]
        ...
        [SerializeField]
        private ExternalPlot externalPlot;
        ...
        {
            externalPlot = gameObject.GetComponent<ExternalPlot>();
            if (externalPlot == null)
            {
                Debug.LogWarning("This component only works with a ExternalPlot.");
                enabled = false;
                return;
            }
        ...
        private void UpdatePosition()
        {
            ...
            switch (connectorFollowType)
            {
                ...
                default:
                    ...
                    // And do nothing else
                    externalPlot.Anchor.transform.position = target.transform.position;
                    break;

                case ConnectorFollowType.Position:
                    // Move the entire tooltip transform while maintaining the anchor position offset
                    externalPlot.transform.position = target.transform.position;
                    switch (PivotMode)
                    {
                        case ConnectorPivotMode.Automatic:
                            ...
                            externalPlot.PivotPosition = target.transform.position + GetDirectionFromPivotDirection(
                                PivotDirection,
                        ...
                        case ConnectorPivotMode.LocalPosition:
                            externalPlot.PivotPosition = target.transform.position + target.transform.TransformPoint(manualPivotLocalPosition);
                            break;
                ...
                case ConnectorFollowType.PositionAndYRotation:
                    ...
                    eulerAngles.x = 0f;
                    eulerAngles.z = 0f;
                    externalPlot.transform.eulerAngles = eulerAngles;
                    ...
                    switch (PivotMode)
                    {
                        case ConnectorPivotMode.Automatic:
                            ...
                            externalPlot.PivotPosition = target.transform.position + localPosition;
                            break;
                ...
                case ConnectorFollowType.PositionAndXYRotation:
                    ...
                    // Set the pivot relative to target/camera
                    externalPlot.transform.position = target.transform.position;
                    externalPlot.transform.rotation = target.transform.rotation;
                    switch (PivotMode)
                    {
                        case ConnectorPivotMode.Automatic:
                            Transform relativeTo = null;
